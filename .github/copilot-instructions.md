...
2. **Ensures remote state backend exists** (`ensure-remote-state.sh`)
   - Creates Azure Storage account if needed (RBAC-only, no shared keys)
   - Configures Terraform backend in `main.tf`
   - Idempotent - safe to run multiple times
3. Executes `terraform init` in `examples/default`
4. Executes `terraform plan`
5. Executes `terraform apply` with auto-approve
6. Outputs Terraform outputs as azd outputs
...
The backend configuration is automatically generated in `backend.tfvars`:
```hcl
...
The `main.tf` contains only an empty backend block, avoiding hardcoded values:
```hcl
terraform {
  backend "azurerm" {}
}
