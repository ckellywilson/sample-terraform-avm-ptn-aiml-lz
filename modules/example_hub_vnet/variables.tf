...
variable "spoke_vnet_resource_ids" {
  ...
  default     = []
  description = <<DESCRIPTION
List of spoke VNet resource IDs to link to private DNS zones.

These VNets will be linked to all private DNS zones created in the hub, enabling DNS resolution
for private endpoints across the hub-spoke topology. This is required for spoke VNets to resolve
private endpoint DNS names (e.g., ACR, Storage, Key Vault) to their private IP addresses.
DESCRIPTION
}
